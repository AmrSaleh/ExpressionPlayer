...
// link to project: http://code.google.com/p/quine-mccluskey/
// This method was written by the original owners in the link above
// This method is used within the Truth Table construction in order to
//    simplify the function. It was modified to suit the needs of the
//    program. Hence, it may not work with a complex expression.
...
public class QuineMC {
    ...
    static String[] dontcares;
    static String check_list[];
    
    
    static String[] pass_dontcare;
    static String[][] track;
    static String[][] track2;
    static String[][] prime_calc;
    
    
   
    static String[][] group_wise;
    
    static String[][] list_track;
    
    static String[] done;
    static String[] undone;
    
    static int term;
    static String[] sub_minterms;
    static int sub_min_index;
    
    static int check_list_index;
    
    static char complete_flag;
    static char first_pass_flag;
    
    static int essential_flag;
    static char first_prime_flag;
    static char stop_flag;
   
    static String[] passed_minterms;
    static String[] on_hold_minterms;
    static int passed_index;
    static int on_hold_index;
    
    static int track_row;
    static int track_col;
   
    static int[] temp_array;
    static int[] int_dontcares;
    static String temp_min2;
    static int bits;
    static int pass_index;
    static String[] first_list;
    static int ones, twos, threes, fours, fives;
    static ArrayList<String> variableNames;
    QuineMC()
    {
    ...
    public String simplify(int[] int_minterms, ArrayList<String> varNames)
    { 	
...
//    	}
    	...
    	{
    		s = "Expression is too long. Unable to simplify.";
    	}
    	return s;
    }
//    public static void main(String[] args) {
//        char if_dontcares;
//        QuineMC test = new QuineMC();
//        variableNames.add("A");
//        variableNames.add("B");
//        variableNames.add("C");
...
public static int[] scan_minterms() {
    ...
    public static String complete_methods() {
        int a = 0, b = 0;
...
first_prime_flag='F';
    ...
    public static String give_output(String[] output_array) {
        ...
        while (!(output_array[i].equals("-1"))) {
            for (int ij = bits - 1; ij >= 0; ij--) {

                if (output_array[i].charAt(ij) != 'x') {

                    if (output_array[i].charAt(ij) == '0') {
                    	for(int mintermNamer = 0; mintermNamer < Math.min(14, variableNames.size()); mintermNamer++)
                    	{
                    		if (ij == mintermNamer) {
                                temp_minterm_check = "~ "+variableNames.get(mintermNamer)+" ";
                            }
                    	}
//                        if (ij == 0) {
...
//                        }
                    ...
                    String old_temp = temp_minterm;
                    temp_minterm = temp_minterm_check + "^ " + temp_minterm;
                    if(old_temp.equals(""))
                    {
                    	temp_minterm=temp_minterm.substring(0, temp_minterm.length()-2);
                    }
                }
            ...
            if (count == 0) {
                minimized_array[minimized_index] = temp_minterm.substring(0, temp_minterm.length());
                minimized_index++;
            }
            temp_minterm = "";
            count = 0;
        ...
        minimized_index = 0;
        while (!(minimized_array[minimized_index].equals("-1"))) {
            output_string = output_string + minimized_array[minimized_index] + "v ";
            if(minimized_array[minimized_index].equals(""))
            {
            	output_string=output_string.substring(0, output_string.length());
            }
            minimized_index++;
        }
        //output_string = output_string.substring(2, output_string.length()-1);
        output_string = fixDuplicateBug(output_string);
    ...
    public static String[][] fill_dontcares(String[][] filling_array) {
        ...
        while (!(pass_dontcare[final_pass_dontcare_index].equals("-1"))) {
    ...
    public static String fixDuplicateBug(String input)
    {
    	String output = "";
    	...
    	{
    		splitter.add(split[i]);
    	}
    	for(int i = 0 ; i < splitter.size(); i++)
    	{
    		//System.out.println(splitter.get(i));
    			
    		for(int j = 0 ; j < splitter.size(); j++)
        	{
        		if(j!=i&&splitter.get(i).equals(splitter.get(j)))
        		{
        			splitter.remove(j);
        			j--;
        		}
        	}
    		output+=splitter.get(i)+" v ";
    	}
